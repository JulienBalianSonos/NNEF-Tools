...
def _same_padding(input, filter, stride, dilation):
    ...
    total = [
        max(0, (di - 1) * s + df - ui)
        for di, s, df, ui in zip(output, stride, dilated, input)
    ]
...
def _positive_pad(input, padding, border="constant", value=0.0):
    ...
    assert all(
        p >= 0 and q >= 0 for p, q in padding
    ), "Negative padding is not supported "

    assert padding
    assert len(input.shape) in (3, 4, 5)
    assert padding[:2] == [(0, 0), (0, 0)] or (
        padding[0] == (0, 0) and padding[-1] == (0, 0)
    )
    assert border in ("constant", "reflect", "replicate")

    rank = len(input.shape)
    needs_transpose = (
        padding[0] == (0, 0) and padding[1] != (0, 0) and padding[-1] == (0, 0)
    )
    if needs_transpose:
        padding = padding[1:-1]
    ...
    padded = (
        F.pad(input=input, pad=pad, mode=border, value=value)
...
def nnef_pad(input, padding, border="constant", value=0.0):
    ...
    assert padding, "nnef.pad does not support empty list as padding"
    assert len(input.shape) in (
        3,
        4,
        5,
    ), "nnef.pad is only implemented for 3D, 4D, 5D tensors; got: {}D.".format(
        len(input.shape)
    )
    assert padding[:2] == [(0, 0), (0, 0)] or (
        padding[0] == (0, 0) and padding[-1] == (0, 0)
    ), "nnef.pad is not implemented in N, C dimensions; got: {}.".format(
        padding
    )
    ...
    assert border in (
        ...
        "reflect",
        "replicate",
    ), "nnef.pad is only implemented with 'constant', 'reflect' and 'replicate' border; got: {}.".format(
        border
    )
...
def nnef_add_n(values):
    return (
        nnef_add(values[0], nnef_add_n(values[1:]))
        if len(values) > 1
        else values[0]
    )
...
def nnef_conv(
    ...
    assert len(input.shape) in (
        ...
        4,
        5,
    ), "nnef.conv is only implemented for 3D, 4D, 5D tensors, given: {}D.".format(
        len(input.shape)
    )

    bias = (
        bias.reshape(1, 1).expand((1, filter.shape[0]))
        if _prod(bias.size()) == 1
        else bias
    )
    ...
    conv = {1: F.conv1d, 2: F.conv2d, 3: F.conv3d}[spatial_dims](
        input=pad,
        weight=filter,
...
def nnef_deconv(
    ...
    assert (
        border == "constant" or border == "replicate"
    ), "nnef.deconv: '{}' border unsupported.".format(border)
    ...
    assert rank in (
        ...
        4,
        5,
    ), "nnef.deconv is only implemented for 3D, 4D, 5D tensors, given: {}D.".format(
        len(input.shape)
    )
    ...
    if output_shape:
        ...
        output_shape = nnef.shapes.deconv_shape(
    ...
    if border == "replicate":
        input = F.pad(
            input=input, pad=(1,) * 2 * spatial_dims, mode="replicate"
        )
        padding = [(p + s, q + s) for (p, q), s in zip(padding, stride)]

    uncropped_output_shape = nnef.shapes.deconv_shape(
        input=list(input.shape),
        filter=filter.shape,
        bias=bias.shape,
        border=border,
    ...
    crop_after = [
        uncropped - out - before
        for uncropped, out, before in zip(
            uncropped_output_shape[2:], output_shape[2:], crop_before
        )
    ]

    bias = (
        bias.reshape(1, 1).expand((1, output_channels))
        if _prod(bias.size()) == 1
        else bias
    )
...
def _evaluate_max_pool_or_box_params(
    input_shape, size, padding, stride, dilation
):
    ...
    padding = (
        _same_padding(
            input=input_shape, filter=size, stride=stride, dilation=dilation
        )
        if not padding
        else padding
    )
...
def _max_pool_impl(
    ...
    result = {1: F.max_pool1d, 2: F.max_pool2d, 3: F.max_pool3d}[spatial_dims](
        input=pad,
        kernel_size=size[2:],
        stride=stride[2:],
        padding=0,
...
def _box_impl(
    ...
    avg_pool = {1: F.avg_pool1d, 2: F.avg_pool2d, 3: F.avg_pool3d}[
        spatial_dims
    ](input=pad, kernel_size=size[2:], stride=stride[2:], padding=0)

    if border == "ignore" and normalize:
        ones = torch.ones_like(input)
        padded_ones = nnef_pad(input=ones, padding=padding, border="constant")
        avg_pool_ones = {1: F.avg_pool1d, 2: F.avg_pool2d, 3: F.avg_pool3d}[
            spatial_dims
        ](input=padded_ones, kernel_size=size[2:], stride=stride[2:], padding=0)
        # If padding is big, zero averages can happen on the border, don't divide by zero
        avg_pool_ones = nnef_select(
            avg_pool_ones > 0, avg_pool_ones, torch.ones_like(avg_pool_ones)
        )
...
def _get_transform_for_box_or_max_pool(input_shape, active):
    ...
    assert (
        sum(active) <= 3
    ), "Sliding window operations are not supported if they have more than 3 'active' dimensions; got {}".format(
        sum(active)
    )

    if (
        3 <= len(input_shape) <= 5 and not active[0] and not active[1]
    ):  # Direct support
...
def _box_or_max_pool(
    ...
    active = [
        size_ != 1 or padding_ != (0, 0) or stride_ != 1 or dilation_ != 1
        for size_, padding_, stride_, dilation_ in zip(
    ...
    if rank < 3:
        ...
        (
            ...
            inactive_shape,
            active_shape,
        ) = _get_transform_for_box_or_max_pool(list(input.shape), active)
    ...
    if rank < 3:
        ...
        output = output.reshape(
            inactive_shape + list(output.shape)[-active_rank:]
        )
...
def nnef_max_pool_with_index(
    ...
    assert len(input_shape) in (
        ...
        4,
        5,
    ), "nnef.max_pool_with_index is only implemented for 3D, 4D, 5D tensors, given: {}D".format(
        len(input_shape)
    )
    assert size[:2] == [
        1,
...
def nnef_debox(
    ...
    assert len(size) in (
        ...
        4,
        5,
    ), "nnef.debox is only implemented for 3D, 4D, 5D tensors, given: {}D".format(
        len(size)
    )
    assert size[:2] == [
        1,
...
def nnef_desample(
    ...
    assert len(input_shape) in (
        ...
        4,
        5,
    ), "nnef.desample is only implemented for 3D, 4D, 5D tensors, given: {}D".format(
        len(input_shape)
    )
    assert not size or size[:2] == [
        1,
        1,
    ...
    if not padding:
        ...
        padding = _same_padding(
            input=calculated_output_shape,
            filter=size,
            stride=stride,
            dilation=dilation,
        )
    else:
        calculated_output_shape = nnef.shapes.desample_shape(
            input_shape,
            index_shape,
            size=size,
            border=border,
    ...
    unpooled = {1: F.max_unpool1d, 2: F.max_unpool2d, 3: F.max_unpool3d}[
        spatial_dims
    ](
        input=input,
        indices=index,
...
def _upsample_weights_1d(factor, symmetric):
    if symmetric:
        ...
        weights = [
            1 - abs(i) / float(factor) for i in range(-factor + 1, factor)
        ]
...
def nnef_multilinear_upsample(
    input, factor, method="symmetric", border="replicate"
):
    ...
    weights = np.tile(
        np.reshape(weights, newshape=(1, 1) + weights.shape),
        reps=(c, 1) + (1,) * rank,
    )
    filter = torch.from_numpy(weights).to(
    ...
    if symmetric:
        ...
        if replicate:
            input = nnef_pad(
                input, padding=[(0, 0), (0, 0)] + [(1, 0)] * rank, border=border
            )
...
def nnef_nearest_upsample(input, factor):
    ...
    assert len(input.shape) in (
        ...
        4,
        5,
    ), "nnef.nearest_upsample is only implemented for 3D, 4D, 5D tensors, given: {}D.".format(
        len(input.shape)
    )

    return F.interpolate(
        input=input, scale_factor=tuple(factor), mode="nearest"
    )
...
def nnef_local_response_normalization(
    input, size, alpha=1.0, beta=0.5, bias=1.0
):
    # type: (torch.Tensor, List[int], float, float, float)->torch.Tensor

    sigma = bias + alpha * nnef_box(
        torch.pow(input, 2.0), size=size, normalize=True
    )
...
def nnef_local_variance_normalization(input, size, bias=0.0, epsilon=0.0):
    ...
    sigma = torch.sqrt(
        nnef_box(torch.pow(input, 2.0), size=size, normalize=True)
    )
    return input / torch.max(
        sigma + bias,
        torch.full(
            size=[], fill_value=epsilon, device=input.device, dtype=input.dtype
        ),
    )
...
def nnef_local_contrast_normalization(input, size, bias=0.0, epsilon=0.0):
    ...
    return nnef_local_variance_normalization(
        centered, size=size, bias=bias, epsilon=epsilon
    )
...
def nnef_matmul(A, B, transposeA=False, transposeB=False):
    ...
    return torch.matmul(
        torch.transpose(A, len(A.shape) - 2, len(A.shape) - 1)
        if transposeA
        else A,
        torch.transpose(B, len(B.shape) - 2, len(B.shape) - 1)
        if transposeB
        else B,
    )
...
def nnef_select(condition, true_value, false_value):
    ...
    rank = max(
        len(condition.shape), len(true_value.shape), len(false_value.shape)
    )
    return torch.where(
        _expand_to_rank(condition, rank),
...
def nnef_sum_reduce(input, axes, normalize=False):
    ...
    return _nnef_generic_reduce(
        input=input, axes=axes, f=torch.mean if normalize else torch.sum
    )
...
def nnef_max_reduce(input, axes):
    ...
    return _nnef_generic_reduce(
        input=input,
        axes=axes,
        f=lambda input, dim, keepdim: torch.max(
...
def nnef_min_reduce(input, axes):
    ...
    return _nnef_generic_reduce(
        input=input,
        axes=axes,
        f=lambda input, dim, keepdim: torch.min(
            input, dim=dim, keepdim=keepdim
        )[0],
    )
...
def _nnef_argminmax_reduce(input, axes, argmin=False):
    ...
    if len(axes) == 1:
        return _nnef_generic_reduce(
            input=input, axes=axes, f=torch.argmin if argmin else torch.argmax
        )
    else:
        axes = sorted(axes)
        ...
        assert (
            axes == consecutive_axes
        ), "{} is only implemented for consecutive axes.".format(
            "argmin_reduce" if argmin else "argmax_reduce"
        )

        reshaped = nnef_reshape(
            input,
            shape=(
                list(input.shape)[: axes[0]]
                + [-1]
                + list(input.shape[axes[0] + len(axes) :])
            ),
        )
        reduced = _nnef_generic_reduce(
            input=reshaped,
            axes=[axes[0]],
            f=torch.argmin if argmin else torch.argmax,
        )
        reshaped = nnef_reshape(
            reduced,
            shape=list(
                dim if axis not in axes else 1
...
def nnef_nearest_downsample(input, factor):
    ...
    return nnef_box(
        input, size=[1] * dims, stride=[1, 1] + factor, padding=[(0, 0)] * dims
    )
...
def nnef_zero_point_linear_quantize(
    x, zero_point, scale, bits, signed, symmetric
):
...
def nnef_gather(input, indices, axis):
    ...
    if len(shape) != 1:
        result = torch.reshape(
            result, shape=input.shape[:axis] + shape + input.shape[axis + 1 :]
        )
    return result
...
Operators = {
    ...
    "cast": nnef_cast,
    "gather": nnef_gather,
    "any_reduce": lambda input, axes: _nnef_generic_reduce(
        input, axes=axes, f=torch.any
    ),
    "all_reduce": lambda input, axes: _nnef_generic_reduce(
        input, axes=axes, f=torch.all
    ),
}
