...
class Converter:
    ...
    def find_public_methods(obj):
        ...
        return {
            name: func for name, func in methods if not name.startswith("_")
        }
    ...
    def decomposed_operations():
        return (
            []
        )  # return list of decomposed NNEF ops in subclass if converting from NNEF

    @staticmethod
    def defined_operations():
        return (
            {}
        )  # return dictionary of NNEF operator (fragment) definitions in subclass if converting to NNEF

    @staticmethod
    def defined_operation_dependencies():
        return (
            {}
        )  # return dictionary of NNEF operator (fragment) dependencies in subclass if converting to NNEF

    @staticmethod
    def defined_shapes():
        return (
            {}
        )  # return dictionary of shape functions for NNEF fragments defined by the converter
    ...
    def unpack_transforms(transforms):
        ...
        for key, value in six.iteritems(transforms):
            ...
            if isinstance(key, tuple):
                if isinstance(value, Transform) and isinstance(
                    value.type, tuple
                ):
    ...
    def __init__(
        ...
        if functions:
            self._callables.update(
                {
                    name: functools.partial(func, self)
    ...
    def __call__(self, graph):
        if not self._infer_shapes:
            unknown_tensors = [
                ...
                if (
                    tensor.shape is None or any(s is None for s in tensor.shape)
                )
            ...
            if len(unknown_tensors):
                names = [
                    "'{}'".format(tensor.name)
                    for tensor in unknown_tensors
                    if tensor.name
                ]
                raise ConversionError(
                    (
                        "Input graph contains tensors with dynamic shape: "
                        + ", ".join(names)
                        if len(names)
                        else "(no names)"
                    )
        ...
        self._tensor_map = {
            tensor: self._copy_tensor_(tensor) for tensor in graph.tensors
        }
        self._tensor_map.update(
            {val: key for key, val in six.iteritems(self._tensor_map)}
        )
        self._transposes = {}
        ...
        if not self._infer_shapes:
            ...
            if len(errors):
                raise ConversionError(
                    "Found {} operator(s) that cannot be converted".format(
                        len(errors)
                    ),
                    details=errors,
                )
            ...
            if self._infer_shapes:
                ...
                for op in self._graph.operations[count:]:
                    ...
                    output_counts = (
                        ...
                        else [None] * len(op.outputs)
                    )
                    output_shapes = _infer_op_shapes(
                        op.type,
                        op.attribs,
                        input_shapes,
                        output_counts,
        ...
        self._graph.remove_tensors(
            [
                tensor
                for tensor in self._graph.tensors
                if len(tensor.producers) == 0 and len(tensor.consumers) == 0
            ]
        )

        self._graph.inputs = tuple(
            self._tensor_map[tensor]
            for tensor in graph.inputs
            if self._tensor_map[tensor].graph
        )
        self._graph.outputs = tuple(
            self._tensor_map[tensor]
    ...
    def tensor_mapping(self):
        return {
            key.name: value.name
            for key, value in six.iteritems(self._tensor_map)
            if value.graph == self._graph
    ...
    def _check_conditions(self, op, transform):
        ...
        op_attribs = self._add_default_attribs(
            op.attribs,
            transform.defaults,
            op_inputs,
            op_outputs,
        ...
        for key, item in six.iteritems(transform.using):
            value = self._evaluate(
                op_attribs, op_inputs, op_outputs, item, using
            )
        ...
        if transform.cond is not None:
            for condition, message in transform.cond.items():
                if not self._evaluate(
                    op_attribs, op_inputs, op_outputs, condition, using
                ):
    ...
    def _error_message(self, op, transform):
        if transform is None:
            return "Conversion of operator '{}' is not implemented".format(
                op.type
            )
        ...
        if message is not None:
            ...
            output_shapes = ", ".join(
                str(tensor.shape) for tensor in op.outputs
            )
            return (
                "Conversion of operator '{}' is not possible: {}"
                "\n  attributes: {}\n  input-shapes: {}\n  output-shapes: {}".format(
                    op.type, message, attribs, input_shapes, output_shapes
                )
            )
    ...
    def _convert(self, op, transform):
        ...
        type = self._evaluate(
            op_attribs, op_inputs, op_outputs, transform.type, using
        )
        self._check_value(type, "field", "type", op.type, op.name)

        name = self._evaluate(
            op_attribs, op_inputs, op_outputs, transform.name, using
        )
        ...
        if isinstance(transform.inputs, list):
            inputs = self._evaluate_tensor_list(
                op_attribs, op_inputs, op_outputs, transform.inputs, using
            )
        ...
        if isinstance(transform.outputs, list):
            outputs = self._evaluate_tensor_list(
                op_attribs, op_inputs, op_outputs, transform.outputs, using
            )
        ...
        for idx, item in enumerate(outputs):
            self._check_value(
                item, "output", idx, op.type, op.name, tensor=True
            )
    ...
    def _mirror(self, op):
        ...
        op_inputs = inputs_type(
            self._tensor_map[tensor] for tensor in op.inputs
        )
        op_outputs = outputs_type(
            self._tensor_map[tensor] for tensor in op.outputs
        )
    ...
    def _add_default_attribs(
        self, attribs, defaults, inputs, outputs, op_type, op_name
    ):
        if defaults is None:
            return attribs
    ...
    def _check_value(self, value, kind, key, op_type, op_name, tensor=False):
        if isinstance(value, Exception):
            raise ConversionError(
                "Could not evaluate {kind} '{key}' while converting operator '{type}'; {err}: {cause}".format(
                    kind=kind,
                    key=key,
                    type=op_type,
                    name=op_name,
                    err=type(value).__name__,
                    cause=str(value) or repr(value),
                )
            )
        if tensor and not isinstance(value, Tensor):
            raise ConversionError(
                "While converting operator '{op_type}', {kind} '{key}' must result in a tensor, "
                "but found {value_type}".format(
                    kind=kind, key=key, op_type=op_type, value_type=type(value)
                )
            )
    ...
    def _copy_tensor_(self, tensor):
        return Tensor(
            ...
            shape=tensor.shape,
            data=tensor.data,
            quant=copy.deepcopy(tensor.quant),
        )
    ...
    def _pre_transpose(self, input, perm):
        ...
        output = Tensor(
            ...
            dtype=input.dtype,
            shape=shape,
            quant=copy.deepcopy(input.quant),
        )
    ...
    def _post_transpose(self, output, perm):
        ...
        input = Tensor(
            ...
            dtype=output.dtype,
            shape=shape,
            quant=copy.deepcopy(output.quant),
        )
        self._transpose_operation(input, output, perm)
        return input
    ...
    def is_zero(self, tensor):
        return (
            self.is_const(tensor)
            and len(tensor.shape) == 0
            and self.as_const(tensor) == 0
        )

    def as_tensor(self, arg, dtype, inline=None):
        return self._make_constant(
            self._graph, dtype=dtype, value=arg, inline=inline
        )
    ...
    def is_integer_upsample(self, input_shape, output_shape):
        return all(
            output % input == 0
            for input, output in zip(input_shape, output_shape)
        )
    ...
    def upsample_factor(self, input_shape, output_shape):
        return [
            output // input for input, output in zip(input_shape, output_shape)
        ]

    def downsample_factor(self, input_shape, output_shape):
        return [
            input // output for input, output in zip(input_shape, output_shape)
        ]
    ...
    def flexible_batch(self, output_shape, batch):
        return (
            [0] + output_shape[1:] if output_shape[0] == batch else output_shape
        )

    def fixed_batch(self, output_shape, batch):
        return (
            [batch] + output_shape[1:] if output_shape[0] == 0 else output_shape
        )
...
class ConverterToNNEF(Converter):
    ...
    def _scale_operation(self, input, output, scalar):
        ...
        Operation(
            input.graph, type="mul", inputs=(input, scalar), outputs=output
        )
    ...
    def remove_unused_constants(graph):
        ops = [
            op
            for op in graph.operations
            if op.type == "constant" and not op.output.has_consumer
        ]
        tensors = [op.output for op in ops]
        graph.outputs = [
            tensor for tensor in graph.outputs if tensor not in tensors
        ]
        ...
        for op in graph.operations:
            if op.type == "constant":
                ...
                if not isinstance(value, np.ndarray):
                    value = np.array(value, op.output.dtype).reshape(
                        op.output.shape
                    )
        ...
        for op in graph.operations:
            if op.type == "constant":
                ...
                if isinstance(value, np.ndarray):
                    ...
                    op.type = "variable"
                    op.attribs["label"] = (
                        op.name if op.name else "variable" + str(variables)
                    )
                    op.output.data = value
...
class ConverterFromNNEF(Converter):
    ...
    def _read_constant(self, tensor, type):
        ...
        return (
            types.from_numpy(value, type=type)
            if isinstance(value, np.ndarray)
            else types.cast(value, type=type)
            if type is not None
            else value
        )
