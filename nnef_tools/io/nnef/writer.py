...
def _print(
    ...
    print(
        "graph {}({}) -> ({})".format(
            graph_name, ", ".join(graph_inputs), ", ".join(graph_outputs)
        ),
        file=file,
    )
    print("{", file=file)
    ...
    for op in graph.operations:
        ...
        attribs = {
            as_str(key): value for key, value in six.iteritems(op.attribs)
        }
...
def _fragment_signature(name, dtype, attribs, inputs, outputs):
    ...
    str += "( "
    str += _types_str(
        ["_I{}".format(i + 1) for i in range(len(inputs))], inputs, True
    )
    ...
    str += " ) -> ( "
    str += _types_str(
        ["_O{}".format(i + 1) for i in range(len(outputs))], outputs, True
    )
    str += " )"
    return str
...
def _types_str(names, items, tensor):
    return ", ".join(
        name
        + ": "
        + ("tensor<{}>".format(type) if tensor else type)
        + ("[]" if repeated else "")
...
class Writer(object):
    ...
    def __call__(self, graph, path):
        ...
        try:
            ...
            fragments = "".join(
                text
                for name, text in six.iteritems(self._fragments)
